...
class Corpus(object):
    ...
    def train_asal(self):
        logger.debug('train asal')
        # train_asal
        logger.debug('.')

        self._embedding = self._embedding.to(opt.device)
        self._embedding.train()

        dataloader = load_asal_dataset(videos=self._videos,
        ...
        self._embedded_feat = self._embedding.embedded(self._embedded_feat.float()).cpu().detach().numpy()
        self._embedded_feat = np.squeeze(self._embedded_feat)
    ...
    def check_ordered(self, selected_pi):
        for i in range(len(selected_pi) - 1):
            if selected_pi[i] > selected_pi[i+1]:
                return False
        return True

    def selected_idx(self, max_z, max_pi, size=3, ordered=False):
        # assume there is no dupicate transcript
        if ordered:
            selected_pi = np.sort(np.random.choice(max_pi, size, replace=False))
        else:
            selected_pi = np.random.choice(max_pi, size, replace=False)
            while self.check_ordered(selected_pi):
                selected_pi = np.random.choice(max_pi, size, replace=False)
        ...
        for a in selected_pi:
            ...
            if max_z[idx+4] == a:
                ...
                selected_z += [idx-4, idx-3, idx-2, idx-1, idx]
